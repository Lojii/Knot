...
class SearchExtensionView: UIView {
    ...
    lazy var segmentedDataSource: JXSegmentedBaseDataSource = {
        ...
        dataSource.titles = [
            ...
            SearchKey.req_target.name(),
            SearchKey.host.name(),
            SearchKey.rsp_state.name(),
            SearchKey.method.name()
        ]
        subListViews = [
            SearchHistoryView(delegate: self),
            SearchListView(task_id: task_id, searchKey: .suffix, delegate: self,tag: 1),// taskID
            SearchListView(task_id: task_id, searchKey: .req_target, delegate: self,tag: 2),
            SearchListView(task_id: task_id, searchKey: .host, delegate: self,tag: 3),
            SearchListView(task_id: task_id, searchKey: .rsp_state, delegate: self,tag: 4),
            SearchListView(task_id: task_id, searchKey: .method, delegate: self,tag: 5)
        ]
    ...
    init(frame:CGRect,task_id:String?) {
...
//    }
    ...
    func updateNumbs(key:SearchKey,numb:Int){
        ...
        if let segDataSource = segmentedDataSource as? JXSegmentedNumberDataSource {
            ...
            if key == .req_target { numbers[2] = numb }
            if key == .host { numbers[3] = numb }
            if key == .rsp_state { numbers[4] = numb }
            if key == .method { numbers[5] = numb }
