...
//        let outImgBtn = setButton(title: "导出图片", y: outRspBtn.frame.maxY + 1, action: #selector(outputHar))
    ...
    func setupScrollView(){
        ...
        let methods = session.method
        if methods != "" { general.append([.method : [methods]]) }
        let type = session.rsp_content_type
        if !type.isEmpty() { general.append([.rsp_content_type : [type]]) }
        let suffix = session.suffix
        if !suffix.isEmpty() { general.append([.suffix : [suffix]]) }
        let state = session.rsp_state
        if state != "" { general.append([.rsp_state : [state]]) }
        let schemes = session.schemes
        if schemes != "" { general.append([.schemes : [schemes]]) }
        let target = session.req_target//Session.getUserAgent(target: )//
        if target != "" { general.append([.req_target : [target]]) }
        if general.count > 0 { items.append(["General":general]) }  //*********************
        // ip/host
        var iphost = [[SearchKey:[String]]]()
        let host = session.host
        if host != "" { iphost.append([.host : [host]]) }
        let local = session.srchost_str
        if local != "" { iphost.append([.srchost_str : [local]])}
        let remote = session.dsthost_str
        if remote != "" { iphost.append([.dsthost_str : [remote]])}
        if iphost.count > 0 { items.append(["IP/Host":iphost]) }  //*********************
        // uri/api
        var uriapi = [[SearchKey:[String]]]()
        let uri = session.uri
        if uri != "" {// www.xxx.com/xxx/xxx/xxx?xx=xx&xx=xx
//            let host = session.host
//            if host != "" {
                if let s1 = uri.components(separatedBy: host).last {    // /xxx/xxx/xxx?xx=xx&xx=xx
                    ...
                    if uriapi.count > 0{ items.append(["URI":uriapi]) }  //*********************
