...
class SessionListViewController: BaseViewController {
    ...
    let littleSearchBar = CGRect(x: 50, y: STATUSBARHEIGHT + (44 - searchBarHeight) / 2, width: SCREENWIDTH - 64 - 50, height: searchBarHeight)
    let bigSearchBar = CGRect(x: LRSpacing, y: STATUSBARHEIGHT + (44 - searchBarHeight) / 2, width: SCREENWIDTH - 64 - LRSpacing, height: searchBarHeight)
    ...
    lazy var searchBar: UITextField = {
        ...
        let icon = UIImageView(frame: CGRect(x: 0, y: 0, width: 30, height: 20))
        icon.image = UIImage(named: "search")
        icon.contentMode = UIView.ContentMode.scaleAspectFit
        leftV.addSubview(icon)
        icon.snp.makeConstraints { make in
            make.centerX.equalTo(leftV.snp_centerX)
            make.centerY.equalTo(leftV.snp_centerY)
        }
    ...
    lazy var searchExtensionView: SearchExtensionView = {
        let searchExtensionView = SearchExtensionView(frame: CGRect(x: 0, y: NAVGATIONBARHEIGHT, width: SCREENWIDTH, height: SCREENHEIGHT - NAVGATIONBARHEIGHT),task_id:task?.task_id)
...
//                searchParams.addMap(key: kv.key, values: kv.value)
    ...
    func loadData(isMore:Bool = false) -> Void {
        let taskID = task?.task_id == nil ? nil : "\(task!.task_id)"
        if !isMore {
            pageIndex = 0
            currentIndex = 0
            ...
            sessions.removeAll()
            updateSQLParams()
            sessionsIDS = Session.countWith(taskID: taskID, keyWord: sqlKeyWord, params: sqlParams, orderBy: nil, timeInterval: currentTime)
            tableView.switchRefreshFooter(to: .normal)
        }
        let results = Session.findAll(taskID: taskID, keyWord: sqlKeyWord, params: sqlParams, pageSize: pageSize, pageIndex: pageIndex, orderBy: nil, timeInterval: currentTime)
...
//        loadData()
    ...
    @objc func nextBtnDidClick(){
        KnotPurchase.check(.HappyKnot) { res in
            if(res){
                ZKProgressHUD.show()
                OutputUtil.output(ids: self.selectedIndexs, type: self.outputType) { (filePath) in
...
extension SessionListViewController:UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row >= 10 && !KnotPurchase.check(.HappyKnot){
            cell.disable = true
        }else{
            cell.disable = false
        }
...
//            print(reqLinePath ?? "no reqLinePath")
//            print(reqHeadPath ?? "no reqHeadPath")
//            print(reqBodyPath ?? "no reqHeadPath")
//
//            print(rspLinePath ?? "no rspLinePath")
//            print(rspHeadPath ?? "no reqHeadPath")
//            print(rspBodyPath ?? "no reqHeadPath")
//            print("---------\(session.conn_id)----------")
