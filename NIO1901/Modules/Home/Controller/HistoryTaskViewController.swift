...
class HistoryTaskViewController: BaseViewController {
    ...
    func loadData(){
        DispatchQueue.global().async {
            ...
            self.tasks = Task.findAll(orderBy: "id")
            self.taskIDS = Task.getAllIds()
            if self.vpnStatus == .connected {
                if let firstId = self.tasks.first?.id {
                    self.tasks.removeFirst()
                    self.taskIDS.removeAll(where: { (id) -> Bool in
                        return firstId.intValue == id
                    })
                }
            }
    ...
    @objc func nextBtnDidClick(){
        ...
        ZKProgressHUD.show()
//
        OutputUtil.taskDoBatch(ids: selectedIndexs, type: outputType) { (filePath) in
            ZKProgressHUD.dismiss()
            
            if filePath == "" {
                if self.outputType == .DEL {
                    ZKProgressHUD.showSuccess("删除成功")
