...
class TaskCell: UITableViewCell {
    ...
    func updateUI(){
        configLabel.text = task?.ruleName
        if let ts = task?.startTime {
            timeLabel.text = "\(Date(timeIntervalSince1970: TimeInterval(truncating: ts)).CurrentStingTime)"
        }
        updataLabel.text = task?.uploadTraffic.floatValue.bytesFormatting()//"\(task?.uploadTraffic ?? 0)B"
        downdataLabel.text = task?.downloadFlow.floatValue.bytesFormatting()//"\(task?.downloadFlow ?? 0)B"
        countLabel.text = "\(task?.interceptCount ?? 0)"
    }
