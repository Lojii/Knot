...
extension VisualActivityViewController {
    static func share(text:String, on viewController:UIViewController,_ suffix:String? = nil){
        var type = ".txt"
        if suffix != nil {
            if suffix != "" {
                type = ".\(suffix!)"
        ...
        let fileUrl = outputFolder.appendingPathComponent("Text-\(Date().dateName)\(type)")
