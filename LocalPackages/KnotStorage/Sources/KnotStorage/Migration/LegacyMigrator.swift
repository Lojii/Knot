...
public enum LegacyMigrator {
    ...
    private static let logger = Logger(subsystem: "KnotStorage", category: "LegacyMigrator")
    ...
    public static func migrateIfNeeded(legacyDBPath: String, rootPath: String) {
        ...
        do {
            ...
            logger.error("Migration failed: \(error.localizedDescription). Backup preserved at \(legacyDBPath).bak")
        }
    }
