...
/// Two paths: async (background queue) and sync (for breakpoint/script intercept).
public class DecodeScheduler {
    ...
    /// Decode synchronously, returning result via EventLoopFuture.
    /// EventLoop is NOT blocked — decode runs on decodeQueue, fulfills promise when done.
    public func decodeSynchronously(flowId: String, eventLoop: EventLoop) -> EventLoopFuture<DecodedPayload> {
        let promise = eventLoop.makePromise(of: DecodedPayload.self)
        decodeQueue.async { [self] in
            do {
