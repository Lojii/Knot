 ...
 */
static ssize_t
log_masterkey_writecb(UNUSED int level, UNUSED void *fh, UNUSED unsigned long ctl,
                      const void *buf, size_t sz)
{
...
log_content_open(log_content_ctx_t *ctx, global_t *global,
                 ...
                 , char *taskId, long long unsigned int connId
	)
{
	char timebuf[24];
	time_t epoch;
	...
	if (ctx->file || ctx->pcap)
		return 0; /* does this actually happen? 有用，多次调用，防止重复打开*/
	...
	if (log_content && global->contentlog) {
		...
		if (global->contentlog_isdir) {
            ...
            char *task_path = malloc(strlen(global->contentlog) + strlen(taskId) + 1);
            asprintf(&task_path, "%s/%s", global->contentlog, taskId);
            if (access(task_path, 0) == -1) {
                if (mkdir(task_path, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) != 0){
                    free(task_path);
                    goto errout;
                }
            }
            free(task_path);
            if (asprintf(&ctx->file->u.dir.req_filename, "%s/%s/%llu.req", global->contentlog, taskId, connId) < 0) {
				log_err_level_printf(LOG_CRIT, "Failed to format filename:" " %s (%i)\n", strerror(errno), errno);
				goto errout;
			}
            if (asprintf(&ctx->file->u.dir.rsp_filename, "%s/%s/%llu.rsp", global->contentlog, taskId, connId) < 0) {
                 log_err_level_printf(LOG_CRIT, "Failed to format filename:" " %s (%i)\n", strerror(errno), errno);
                 goto errout;
             }
...
log_content_file_dir_opencb(void *fh)
{
    ...
    if ((ctx->u.dir.req_fd = privsep_client_openfile(content_file_clisock,
                                                 ctx->u.dir.req_filename,
                                                 0)) == -1) {
        log_err_level_printf(LOG_CRIT, "Opening logdir req_file '%s' failed: %s (%i)\n",
                       ctx->u.dir.req_filename,
                       strerror(errno), errno);
        return -1;
    }
    if ((ctx->u.dir.rsp_fd = privsep_client_openfile(content_file_clisock,
                                                 ctx->u.dir.rsp_filename,
                                                 0)) == -1) {
        log_err_level_printf(LOG_CRIT, "Opening logdir rsp_file '%s' failed: %s (%i)\n",
                       ctx->u.dir.rsp_filename,
                       strerror(errno), errno);
        return -1;
    }
...
log_content_file_dir_closecb(void *fh, UNUSED unsigned long ctl)
{
	...
	if (ctx->u.dir.req_filename)
		free(ctx->u.dir.req_filename);
	if (ctx->u.dir.req_fd != 1)
		close(ctx->u.dir.req_fd);
    if (ctx->u.dir.rsp_filename)
        free(ctx->u.dir.rsp_filename);
    if (ctx->u.dir.rsp_fd != 1)
        close(ctx->u.dir.rsp_fd);
	free(ctx);
}

static ssize_t
log_content_file_dir_writecb(UNUSED int level, void *fh, unsigned long ctl,
                             const void *buf, size_t sz)
{
    
    
    int direction = (ctl & LBFLAG_IS_REQ) ? LOGPKT_REQUEST : LOGPKT_RESPONSE;
	log_content_file_ctx_t *ctx = fh;
    if(!direction){
        if (write(ctx->u.dir.req_fd, buf, sz) == -1) {
            log_err_level_printf(LOG_CRIT, "Failed to write to content req_log: %s\n",
                           strerror(errno));
            return -1;
        }
    }else{
        if (write(ctx->u.dir.rsp_fd, buf, sz) == -1) {
            log_err_level_printf(LOG_CRIT, "Failed to write to content rsp_log: %s\n",
                           strerror(errno));
            return -1;
        }
    }
...
static logbuf_t *
log_content_file_prepcb(UNUSED void *fh, unsigned long prepflags,
                        logbuf_t *lb) {
    if (prepflags & PREPFLAG_EOF)
        return lb;
    logbuf_ctl_set(lb, (prepflags & PREPFLAG_REQUEST) ? LBFLAG_IS_REQ : LBFLAG_IS_RESP);
    return lb;
}
...
log_content_pcap_preinit(const char *pcapfile)
{
	...
	if (logpkt_pcap_open_fd(content_pcap_fd) == -1) {
		log_err_level_printf(LOG_CRIT, "Failed to prepare '%s' for PCAP writing"
		               ": %s (%i)\n",
		               pcapfile, strerror(errno), errno);
...
log_content_pcap_writecb_base(void *fh, unsigned long ctl,
	...
	return sz;
errout:
	log_err_level_printf(LOG_CRIT, "Failed to write to pcap log: %s (%i)\n",
	               strerror(errno), errno);
	return -1;
}

static ssize_t
log_content_pcap_writecb(UNUSED int level, void *fh, unsigned long ctl,
                         const void *buf, size_t sz) {
...
log_content_pcap_dir_opencb(void *fh)
{
	...
	if ((ctx->u.dir.fd = privsep_client_openfile(content_pcap_clisock,
	                                             ctx->u.dir.filename,
	                                             0)) == -1) {
		log_err_level_printf(LOG_CRIT, "Opening pcapdir file '%s' failed: %s (%i)\n",
...
static ssize_t
log_content_pcap_dir_writecb(UNUSED int level, void *fh, unsigned long ctl,
                             const void *buf, size_t sz)
{
	log_content_pcap_ctx_t *ctx = fh;
...
log_content_pcap_spec_opencb(void *fh)
{
	...
	if ((ctx->u.spec.fd = privsep_client_openfile(content_pcap_clisock,
	                                              ctx->u.spec.filename,
	                                              1)) == -1) {
		log_err_level_printf(LOG_CRIT, "Opening pcapspec file '%s' failed: %s (%i)\n",
		               ctx->u.spec.filename, strerror(errno), errno);
		return -1;
	}
...
static ssize_t
log_content_pcap_spec_writecb(UNUSED int level, void *fh, unsigned long ctl,
                              const void *buf, size_t sz)
{
	log_content_pcap_ctx_t *ctx = fh;
...
static ssize_t
log_cert_writecb(UNUSED int level, void *fh, UNUSED unsigned long ctl,
                 const void *buf, size_t sz)
{
	char *fn = fh;
	int fd;
...
/*
 * Log pre-init: open all log files but don't start any threads, since we may fork() after pre-initialization.
