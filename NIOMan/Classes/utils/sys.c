...
/*
 * Write process ID to open process ID file descriptor fd.将进程ID写入进程ID文件描述符fd。
...
/*
 * Converts an IPv4/IPv6 sockaddr into printable string representations of the host and the service (port) part.  Writes allocated buffers to *host and *serv which must both be freed by the caller.  Neither *host nor *port are freed by this function before newly allocating.
 * Returns 0 on success, -1 otherwise.  When -1 is returned, pointers in *host and *serv are invalid and must not be used nor freed by the caller.
 将IPv4/IPv6 sockaddr转换为主机和服务(端口)部分的可打印字符串表示形式。将已分配的缓冲区写入*host和*serv，调用者必须释放这两个缓冲区。在新分配之前，*host和*port都不会被释放。
 */
int
...
/*
 * Iterate over all files in a directory hierarchy, calling the callback cb for each file, passing the filename and arg as arguments.  Files and directories beginning with a dot are skipped, symlinks are followed.
 * 遍历目录层次结构中的所有文件，为每个文件调用回调函数cb，将文件名和参数作为参数传递。以点开头的文件和目录被跳过，符号链接紧随其后。
 */
int
