...
public class NetworkInfo {
    ...
    public static func LocalWifiIPv4() -> String? {
        guard let list = IPAddressList() else { return nil }
        guard let i = list.firstIndex(where: { $0.device == Device.LocalWifi.rawValue && $0.version == .iPv4 }) else { return nil }
        return list[i].IP
    }
    
    public static func WanIPv4() -> String? {
        guard let list = IPAddressList() else { return nil }
        guard let i = list.firstIndex(where: { $0.device == Device.Wan.rawValue && $0.version == .iPv4 }) else { return nil }
        return list[i].IP
    }
}
